...
Learn Guide: https://learn.adafruit.com/raspberry-pi-single-channel-lorawan-g$
...
print('Gateway ID: {0}:{1}:{2}:ff:ff:{3}:{4}:{5}'.format(mac_addr[0:2],mac_ad$
                                                         mac_addr[4:6],mac_ad$
                                                         mac_addr[8:10], mac_$
...
def stats():
    ...
    # https://unix.stackexchange.com/questions/119126/command-to-display-memo$
    cmd = "hostname -I | cut -d\' \' -f1"
    IP = subprocess.check_output(cmd, shell=True).decode("utf-8")
    cmd = "top -bn1 | grep load | awk '{printf \"CPU Load: %.2f\", $(NF-2)}'"
    CPU = subprocess.check_output(cmd, shell=True).decode("utf-8")
    cmd = "free -m | awk 'NR==2{printf \"Mem: %s/%s MB  %.2f%%\", $3,$2,$3*10$
...
def gateway():
    ...
    try:
        proc = subprocess.Popen("./single_chan_pkt_fwd",
                                bufsize=-1, stdout=subprocess.PIPE, stderr=su$
    except FileNotFoundError:
        print("To run the single packet forwarder, you'll need to run `sudo m$
        return
    #display.fill(0)
    ...
    while True:
        ...
        if new_line == "gateway status update\n":
            ...
            #display.text('RSSI: {0}dBm, Sz: {1}b'.format(pkt_rssi, pkt_size)$
...
def gateway_info():
    ...
    #display.text(gateway_name, 15, 0, 1)
    #display.text('{0} MHz, SF{1}'.format(gateway_freq, gateway_sf), 15, 10, $
